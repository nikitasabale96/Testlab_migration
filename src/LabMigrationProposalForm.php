...
class LabMigrationProposalForm extends FormBase {
  ...
  public function buildForm(array $form, \Drupal\Core\Form\FormStateInterface $form_state) {
    ...
    if ($user->uid == 0) {
      $msg = add_message(t('It is mandatory to ' . Link::fromTextAndurl('login', 'user') . ' on this website to access the lab proposal form. If you are new user please create a new account first.'), 'error');
      //RedirectResponse('lab-migration-project');
      RedirectResponse('user');
      return $msg;
    }
    $query = $injected_databse->select('lab_migration_proposal');
    ...
    if ($proposal_data) {
      if ($proposal_data->approval_status == 0 || $proposal_data->approval_status == 1) {
        add_message(t('We have already received your proposal.'), 'status');
        RedirectResponse('');
        return;
      }
    }
    ...
    $form['syllabus_copy']['syllabus_copy_file'] = [
      ...
      '#size' => 48,
      '#description' => t('Separate filenames with underscore. No spaces or any special characters allowed in filename.') . '<br />' . t('<span style="color:red;">Allowed file extensions : ') . $config->get('lab_migration_syllabus_file_extensions', '') . '</span>',
    ];
    ...
    if (isset($_FILES['files'])) {
      ...
      foreach ($_FILES['files']['name'] as $file_form_name => $file_name) {
        if ($file_name) {
          ...
          switch ($file_type) {
            case 'S':
              $allowed_extensions_str = $config->get('lab_migration_syllabus_file_extensions', '');
              break;
          } //$file_type
  ...
  public function submitForm(array &$form, \Drupal\Core\Form\FormStateInterface $form_state) {
    $user = \Drupal::currentUser();
    if (!$user->uid) {
      add_message('It is mandatory to login on this website to access the proposal form', 'error');
      return;
    }
    ...
    foreach ($_FILES['files']['name'] as $file_form_name => $file_name) {
      if ($file_name) {
        ...
        if (move_uploaded_file($_FILES['files']['tmp_name'][$file_form_name], $root_path . $dest_path . $_FILES['files']['name'][$file_form_name])) {
          ...
          $updateresult = $injected_database->query($query, $args);
    ...
    if (!$proposal_id) {
      add_message(t('Error receiving your proposal. Please try again.'), 'error');
      return;
    }
    /* proposal id */
    ...
    for ($counter = 1; $counter <= 15; $counter++) {
      ...
      if (strlen(trim($form_state->getValue([$experiment_field_name]))) >= 1) {
        ...
        if (!$result) {
          add_message(t('Could not insert Title of the Experiment : ') . trim($form_state->getValue([$experiment_field_name])), 'error');
        }
        else {
          $number++;
        }
    ...
    add_message(t('We have received you Lab migration proposal. We will get back to you soon.'), 'status');
    RedirectResponse('');
  }
