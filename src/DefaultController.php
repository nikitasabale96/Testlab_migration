...
class DefaultController extends ControllerBase {
  ...
  public function lab_migration_proposal_pending() {
    ...
    while ($pending_data = $pending_q->fetchObject()) {
      ...
      fromTextAndUrl($pending_data->name, 'user/' . $pending_data->uid),
  ...
  public function lab_migration_solution_proposal_pending() {
    ...
    while ($pending_data = $pending_q->fetchObject()) {
      $pending_rows[$pending_data->id] = [
        fromTextAndUrl($pending_data->name, 'user/' . $pending_data->uid),
        $pending_data->lab_title,
        fromTextAndUrl('Approve', 'lab-migration/manage-proposal/solution-proposal-approve/' . $pending_data->id),
      ];
    }
    /* check if there are any pending proposals */
    if (!$pending_rows) {
  ...
  public function lab_migration_proposal_pending_solution() {
    ...
    fromTextAndUrl($pending_data->name, 'user/' . $pending_data->uid),
        $pending_data->lab_title,
        $pending_data->department,
        fromTextAndUrl('Status', 'lab-migration/manage-proposal/status/' . $pending_data->id),
      ];
    }
    /* check if there are any pending proposals */
    if (!$pending_rows) {
  ...
  public function lab_migration_proposal(){
    ...
    while ($proposal_data = $proposal_q->fetchObject()) {
      ...
      $proposal_rows[] = [
        ...
        fromTextAndUrl($proposal_data->name, 'user/' . $proposal_data->uid),
  ...
  public function lab_migration_category_proposal() {
    ...
    while ($proposal_data = $proposal_q->fetchObject()) {
      $proposal_rows[] = [
        ...
        fromTextAndUrl('Edit Category', 'lab-migration/manage-proposal/category/edit/' . $proposal_data->id),
      ];
    }
    $proposal_header = [
      'Date of Submission',
  ...
  public function lab_migration_proposal_open() {
    ...
    if ($proposal_q_count != 0) {
      while ($proposal_data = $proposal_q->fetchObject()) {
        $proposal_rows[] = [
        fromTextAndUrl($proposal_data->lab_title, 'lab-migration/show-proposal/' . $proposal_data->id),
        fromTextAndUrl('Apply', 'lab-migration/show-proposal/' . $proposal_data->id),
        ];
      }
      $proposal_header = [
        'Title of the Lab',
  ...
  public function lab_migration_code(){
    ...
    while ($pending_solution_data = $pending_solution_q->fetchObject()) {
      ...
      $pending_solution_rows[] = [
        ...
        $proposal_data->name,
        $solution_provider_user_name,
        fromTextAndUrl('Edit', 'lab-migration/code-approval/approve/' . $pending_solution_data->id),
      ];
    }
  ...
  public function lab_migration_list_experiments() {
    ...
    $return_html .= fromTextAndUrl('Upload Solution', 'lab-migration/code/upload') . '<br />';
        ...
        while ($solution_data = $solution_q->fetchObject()) {
          ...
          if ($solution_data->approval_status == 0) {
            $experiment_rows[] = [
              ...
              '',
              $solution_status,
              fromTextAndUrl('Delete', 'lab-migration/code/delete/' . $solution_data->id),
            ];
          }
          else {
            $experiment_rows[] = [
          ...
          if ($solution_files_q) {
            while ($solution_files_data = $solution_files_q->fetchObject()) {
              ...
              $experiment_rows[] = [
                "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" . fromTextAndUrl($solution_files_data->filename, 'lab-migration/download/file/' . $solution_files_data->id),
                $code_file_type,
                '',
                '',
              ];
          ...
          while ($dependency_data = $dependency_q->fetchObject()) {
            ...
            $experiment_rows[] = [
              "&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;" . fromTextAndUrl($dependency_files_data->filename, 'lab-migration/download/dependency/' . $dependency_files_data->id),
              'Dependency',
              '',
              '',
            ];
    ...
    if ($result->rowCount() == 0) {
      ...
      while ($row = $result->fetchObject()) {
        ...
        $preference_rows[] = [
          $i,
          $row->university,
          fromTextAndUrl($row->lab_title, "lab-migration/lab-migration-run/" . $row->id),
          $approval_date,
        ];
        $i--;
      }
  ...
  public function lab_migration_labs_progress_praposal(){
    $page_content = "";
    ...
    foreach ($details_list as $details) {
      if ($details->type == "Proposer") {
        $search_rows[] = [
          ...
          $details->name,
          $details->type,
          fromTextAndUrl('Download Certificate', 'lab-migration/certificate/generate-pdf/' . $details->proposal_id . '/' . $details->id),
          fromTextAndUrl('Edit Certificate', 'lab-migration/certificate/lm-proposer/form/edit/' . $details->proposal_id . '/' . $details->id),
        ];
        ...
        $search_rows[] = [
          ...
          $details->type,
          fromTextAndUrl('Download Certificate', 'lab-migration/certificate/generate-pdf/' . $details->proposal_id . '/' . $details->id),
            fromTextAndUrl('Edit Certificate', 'lab-migration/certificate/lm-participation/form/edit/' . $details->proposal_id . '/' . $details->id),
        ];
      }
